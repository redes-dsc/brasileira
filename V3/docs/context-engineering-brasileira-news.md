...
**Escopo:** Aplicação prática de context engineering na migração multi-agente do brasileira.news  
...
Para o brasileira.news — um sistema multi-agente com 10+ papéis editoriais hierárquicos (Diretor de Redação → Editores → Repórteres → Focas), processando 1.000+ artigos/dia a partir de 630+ fontes, com roteamento LLM de 6 tiers via LiteLLM — context engineering não é opcional. É a diferença entre agentes que produzem o milésimo artigo com a mesma qualidade do primeiro e agentes que degradam progressivamente por context rot, envenenamento de contexto ou confusão entre ferramentas ([Firecrawl](https://www.firecrawl.dev/blog/context-engineering)).
...
┌────────────────────────────────────────────────────────────┐
│  DIRETOR DE REDAÇÃO                                         │
│  Contexto: Plano editorial + métricas macro + tendências    │
│  Ferramentas: consultar_metricas, definir_prioridades,      │
│               avaliar_performance                           │
│  Janela: ~8K tokens (leve, decisório)                       │
├────────────────────────────────────────────────────────────┤
│  EDITOR-CHEFE                                               │
│  Contexto: Plano do diretor + artigos candidatos + quotas   │
│  Ferramentas: distribuir_pautas, aprovar_materia,           │
│               gerenciar_fila                                │
│  Janela: ~16K tokens (coordenação)                          │
├────────────┬────────────┬────────────┬─────────────────────┤
│ EDITOR     │ EDITOR     │ EDITOR     │ EDITOR              │
│ Política   │ Economia   │ Esportes   │ Tecnologia          │
│ Ctx: sua   │ Ctx: sua   │ Ctx: sua   │ Ctx: sua            │
│ editoria   │ editoria   │ editoria   │ editoria            │
│ apenas     │ apenas     │ apenas     │ apenas              │
│ ~12K tok   │ ~12K tok   │ ~12K tok   │ ~12K tok            │
├────────────┴────────────┴────────────┴─────────────────────┤
│  REPÓRTERES / REDATORES / FOCAS                             │
│  Contexto: 1 pauta + fontes primárias + guidelines          │
│  Ferramentas: coletar_fonte, redigir_artigo, buscar_imagem  │
│  Janela: ~32K tokens (trabalho pesado, isolado por matéria) │
└────────────────────────────────────────────────────────────┘
```
...
- [ ] Definir schema de estado global no LangGraph
- [ ] Implementar camadas de memória: Redis (trabalho) + PostgreSQL/pgvector (longo prazo)
- [ ] Criar template de system prompt padronizado com XML tags
- [ ] Implementar contratos JSON para todos os outputs inter-agente
- [ ] Migrar `historico_links.txt` para tabela indexada
- [ ] Configurar single source of truth para configurações (`editorial_config.yaml`)
- [ ] Eliminar todas as credenciais hardcoded

### Fase 2 — Estratégias Core (Semanas 5-8)

- [ ] Implementar scratchpads em Redis para cada papel de agente
- [ ] Criar pipeline de recuperação híbrida (semântica + lexical + estrutural)
...
- [ ] Deploy da hierarquia multi-agente com janelas isoladas
- [ ] Implementar sandbox para pipeline de imagens
- [ ] Configurar transição atômica da homepage
- [ ] Implementar locks distribuídos para deduplicação
